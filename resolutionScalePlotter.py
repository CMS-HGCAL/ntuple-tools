...
def extractResolutionScale(resolutionFileAndInfoMap, renorm):
    if (verbosityLevel >= 1):
    ...
    for etaBinName in etaBins:
        ...
        for phiBinName in phiBins:
            ...
            resScale_values[etaBinName][phiBinName]['effSigma'] = effSigma * renorm[etaBinName][phiBinName]
            # print basic info on scale/resolution
            if (verbosityLevel >= 1):
                "{0:.2f}".format(gMean) + " +/- " + "{0:.2f}".format(gMeanError), "\t", GEN_eta, "\t", GEN_phi, "\t", "(pf clusters: " + \
                    str(int(hEntries)) + ")", "\t", "effSigma: ", effSigma, "\t", "gStd: ", gStd
...
def setupSummaryGraphs(pidSelected, GEN_engpts, resScale_values, scenarios):
    ...
    for scenario in scenarios:
        if (scenario == "PF_PU200"):  # for scenario PF_PU200
            ...
            graphsAndProps[ROOT.TGraph(len(xEng), xEng, yRes)] = {"leg": "PF cluster, pileup 200", "color": ROOT.kBlue, "MarkerStyle": 21, "LineStyle": 4}
        elif (scenario == "PF_noPU"):  # for scenario PF_noPU
            xEng = array('f', resScale_values[scenario].keys())
            yRes = array('f', [resScale_values[scenario][engpt]['eta1p479to3p0']['phim1p0pito1p0pi']['effSigma'] for engpt in resScale_values[scenario].keys()])
            graphsAndProps[ROOT.TGraph(len(xEng), xEng, yRes)] = {"leg": "PF cluster, no pileup", "color": ROOT.kGreen - 6, "MarkerStyle": 21, "LineStyle": 4}
        elif (scenario == "Mega_PU200"):  # for scenario PF_PU200
            xEng = array('f', resScale_values[scenario].keys())
            yRes = array('f', [resScale_values[scenario][engpt]['eta1p479to3p0']['phim1p0pito1p0pi']['effSigma'] for engpt in resScale_values[scenario].keys()])
            graphsAndProps[ROOT.TGraph(len(xEng), xEng, yRes)] = {"leg": "Megacluster, pileup 200", "color": ROOT.kRed - 6, "MarkerStyle": 25, "LineStyle": 1}
        elif (scenario == "Mega_noPU"):  # for scenario PF_noPU
            xEng = array('f', resScale_values[scenario].keys())
            yRes = array('f', [resScale_values[scenario][engpt]['eta1p479to3p0']['phim1p0pito1p0pi']['effSigma'] for engpt in resScale_values[scenario].keys()])
            graphsAndProps[ROOT.TGraph(len(xEng), xEng, yRes)] = {"leg": "Megacluster, no pileup", "color": ROOT.kBlack, "MarkerStyle": 20, "LineStyle": 1}
...
def main():
        ...
        for scenario in scenarios:
            ...
            for GEN_engpt in GEN_engpts:
                ...
                if len(resolutionFileAndInfoMap['file'].GetListOfKeys()) > 0:
                    ...
                    fileMega_noPU_norm = ROOT.TFile.Open("{}_{}_{}GeV_{}_{}_{}.root".format(gun_type, pidSelected, int(GEN_engpt), refName, "megacluster", "noPU"), "read")  # info based on PF energy
                    # map of histograms and files
                    renorm = {}
                    for etaBinName in etaBins:
                        renorm[etaBinName] = {}
                        for phiBinName in phiBins:
                            hist_nopup_tmp = fileMega_noPU_norm.Get("obj_Pt_eta" + etaBinName + "_phi" + phiBinName)  # get resolution/scale histogram
                            renorm[etaBinName][phiBinName] = GEN_engpt / hist_nopup_tmp.GetMean()

                    resScale_values[pidSelected][scenario][GEN_engpt] = extractResolutionScale(resolutionFileAndInfoMap, renorm)
                    # prepare/plot/save histograms for comparison (e, pt, etc.)
    ...
    for pidSelected in pids:
        ...
        for gr in graphsAndProps:
            (gr, stohasticTerm, constantTerm, noiseTerm) = hgcalHistHelpers.fitResolution(gr, graphsAndProps[gr]['color'])
        ...
        hgcalHistHelpers.drawGraphs(graphsAndProps, outDir, title="Energy resolution (" +
                                    pidmap[pidSelected] + "s, |#eta| #in [1.6 - 2.8])", tag="resolutionCmp_" + pidmap[pidSelected] + "_scenarios_" + "_".join(scenarios) + "_" + tag)
