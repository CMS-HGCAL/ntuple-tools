...
{
	if(argc != 3){
		cout<<"Usage: output_path param_name step"<<endl;
	...
	string testParam = argv[2];
	double testParamStep = 0.5;//stod(argv[3]);
	...
	auto scoreVsParamHist = my::make_unique<TH1D>(("score_vs_"+testParam).c_str(),
																								("score_vs_"+testParam).c_str(),
																								(paramMax[paramNum]-paramMin[paramNum])/testParamStep+1,
																								paramMin[paramNum], paramMax[paramNum]);
	
	// Copy default config
	configPath = outputPath+"/testScore_"+testParam+".md";
	system(("cp "+outputPath+"/"+baseConfigPath+" "+configPath).c_str());
	
	// Put random score output path in the random config
	string scoreOutputPath = outputPath+"/testScore_"+testParam+".out";
	...
	scoreVsParamHist->SaveAs((outputPath+"/score_vs_"+testParam+".root").c_str());
	return 0;
}
